...
<xsl:text>タイトル：</xsl:text>
   <xsl:value-of select="../xhtml:body/xhtml:h1"/>
 <b />
...
<xsl:text>翻訳者：</xsl:text>
  <xsl:if test="/xhtml:html/xhtml:body/xhtml:h2[@class='translator']">
   <xsl:value-of select="/xhtml:html/xhtml:body/xhtml:h2[@class='translator']"/>
  </xsl:if>
 <b />

<xsl:text>編者：</xsl:text>
  <xsl:if test="/xhtml:html/xhtml:body/xhtml:h2[@class='editor']">
   <xsl:value-of select="/xhtml:html/xhtml:body/xhtml:h2[@class='editor']"/>
  </xsl:if>
 <b />
...
<xsl:template match="xhtml:div[@class='bibliographical_information']">
       <xsl:text disable-output-escaping="yes">&lt;</xsl:text>フッタ<xsl:text disable-output-escaping="yes">&gt;</xsl:text><ブロック 種別="書誌情報"><xsl:apply-templates/></ブロック>
</xsl:template>

<xsl:template match="xhtml:div[@class='notation_notes']">
       <ブロック 種別="表記について"><xsl:apply-templates/></ブロック>
...
<xsl:text disable-output-escaping="yes">&lt;</xsl:text>/フッタ<xsl:text disable-output-escaping="yes">&gt;</xsl:text>
